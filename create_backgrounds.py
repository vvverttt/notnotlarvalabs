...
# Create background for Ether Rocks
create_background_image(
    'ether_rocks_original',
    'images/eths-rock-new.png',
    width=1920,
    height=480,
    grid_cols=50,
    grid_rows=6
...
# Create background for CryptoPhunks v69 (first images only)
def create_background_from_first_images(image_folder, output_filename, width=1920, height=480, grid_cols=50, grid_rows=6, num_images=50):
    """Create a background image using only the first N images from the folder"""
    
    # Get all PNG files from the folder and take only the first num_images
    image_files = [f for f in os.listdir(image_folder) if f.endswith('.png')][:num_images]
    ...
    # Save the background image
    background.save(output_filename, 'PNG')
    print(f"Created {output_filename} with {total_cells} phunks (using first {len(image_files)} images)")

# Create background for CryptoPhunks v69 (first images only)
create_background_from_first_images(
    'cryptophunksv69',
    'images/cryptophunksv69-new.png',
    width=1920,
    height=480,
    grid_cols=50,
    grid_rows=6,
